...
restaurantMenuRoutes.delete('/menu/products/:id',requirePermission('orders.write'),async c=>{
  const productId=id.safeParse(c.req.param('id'));if(!productId.success)return error(c,422,'VALIDATION_ERROR','Item inválido');const r=await c.get('db').execute(sql`update products set active=false,updated_at=now() where id=${productId.data}::uuid and business_id=${c.get('auth').businessId}::uuid returning id`);if(!rows(r).length)return error(c,404,'NOT_FOUND','Item não encontrado');return c.json({data:{id:productId.data,active:false}});
});

restaurantMenuRoutes.post('/menu/products/:id/image',requirePermission('orders.write'),async c=>{
  const productId=id.safeParse(c.req.param('id'));if(!productId.success)return error(c,422,'VALIDATION_ERROR','Item inválido');
  const product:any=rows(await c.get('db').execute(sql`select id,primary_image_file_id from products where id=${productId.data}::uuid and business_id=${c.get('auth').businessId}::uuid limit 1`))[0];if(!product)return error(c,404,'NOT_FOUND','Item não encontrado');
  const form=await c.req.formData().catch(()=>null);const file=form?.get('file');if(!(file instanceof File))return error(c,422,'VALIDATION_ERROR','Imagem obrigatória');if(file.size<=0||file.size>MAX_FILE_BYTES)return error(c,413,'FILE_TOO_LARGE','A imagem deve ter no máximo 10 MB');if(!imageTypes.has(file.type))return error(c,415,'UNSUPPORTED_MEDIA_TYPE','Use JPG, PNG, WEBP, GIF ou AVIF');
  const businessId=c.get('auth').businessId,db=c.get('db');const quota=await db.execute(sql`select coalesce(sum(size_bytes),0) used from files where business_id=${businessId}::uuid and deleted_at is null`);const configured=await db.execute(sql`select storage_limit_bytes from business_quotas where business_id=${businessId}::uuid limit 1`);const used=Number((rows(quota)[0] as any)?.used??0),limit=Number((rows(configured)[0] as any)?.storage_limit_bytes??100*1024*1024);if(used+file.size>limit)return error(c,409,'STORAGE_QUOTA_EXCEEDED','Limite de armazenamento atingido',{usedBytes:used,limitBytes:limit});
  const fileId=uuidv7(),linkId=uuidv7(),ext=(file.name.split('.').pop()??'bin').toLowerCase().replace(/[^a-z0-9]/g,'').slice(0,8)||'bin';const key=`businesses/${businessId}/product/${productId.data}/${fileId}.${ext}`;const buffer=await file.arrayBuffer();const hash=await crypto.subtle.digest('SHA-256',buffer);const checksum=[...new Uint8Array(hash)].map(x=>x.toString(16).padStart(2,'0')).join('');
  ...
  try{await db.execute(sql`insert into files(id,business_id,bucket,object_key,original_name,mime_type,size_bytes,checksum_sha256,visibility,uploaded_by,purpose) values(${fileId},${businessId}::uuid,'r2',${key},${file.name.slice(0,255)},${file.type},${file.size},${checksum},'public',${c.get('auth').userId}::uuid,'menu-primary')`);await db.execute(sql`insert into entity_files(id,business_id,file_id,entity_type,entity_id,purpose,created_by) values(${linkId},${businessId}::uuid,${fileId}::uuid,'product',${productId.data}::uuid,'menu-primary',${c.get('auth').userId}::uuid)`);await db.execute(sql`update products set primary_image_file_id=${fileId}::uuid,updated_at=now() where id=${productId.data}::uuid and business_id=${businessId}::uuid`)}catch(e){await c.env.R2_BUCKET.delete(key);throw e;}
  ...
  return c.json({data:{id:fileId,imageUrl:new URL(`/api/public/media/${fileId}`,c.req.url).toString()}},201);
});
