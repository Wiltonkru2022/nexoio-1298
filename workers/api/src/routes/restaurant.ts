...
import { cashMovements, cashSessions, inventoryMovements, kitchenTickets, restaurantOrderItems, restaurantOrders, restaurantPayments, restaurantTables, restaurantTabs, saleItems, salePayments, sales } from '@nexoio/db';
...
restaurantRoutes.post('/tabs/:id/orders',async c=>{
 ...
 const allOrders=await db.select({id:restaurantOrders.id}).from(restaurantOrders).where(and(eq(restaurantOrders.businessId,b),eq(restaurantOrders.tabId,id))); const allItems=allOrders.length?await db.select().from(restaurantOrderItems).where(and(eq(restaurantOrderItems.businessId,b),inArray(restaurantOrderItems.orderId,allOrders.map(o=>o.id)))):[]; const subtotal=allItems.reduce((s,i)=>s+n(i.total),0); const discount=n(tab[0].discount),total=Math.max(0,subtotal-discount); await db.update(restaurantTabs).set({subtotal:subtotal.toFixed(2),total:total.toFixed(2),status:'active',updatedAt:new Date()}).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b))); return c.json({data:{orderId,subtotal,total}},201);
});
restaurantRoutes.post('/tabs/:id/request-closure',async c=>{const b=c.get('auth').businessId,id=c.req.param('id'),db=c.get('db');const tab=await db.select().from(restaurantTabs).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b))).limit(1);if(!tab[0])return error(c,404,'NOT_FOUND','Comanda não encontrada');await db.update(restaurantTabs).set({status:'awaiting_closure',requestedClosureAt:new Date(),updatedAt:new Date()}).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b)));if(tab[0].tableId)await db.update(restaurantTables).set({status:'closing',updatedAt:new Date()}).where(and(eq(restaurantTables.id,tab[0].tableId),eq(restaurantTables.businessId,b)));return c.json({data:{ok:true}})});
restaurantRoutes.post('/tabs/:id/payments',async c=>{const b=c.get('auth').businessId,id=c.req.param('id'),db=c.get('db'),body=await c.req.json().catch(()=>({}));const tab=await db.select().from(restaurantTabs).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b))).limit(1);if(!tab[0])return error(c,404,'NOT_FOUND','Comanda não encontrada');const amount=Math.max(0,n(body.amount));if(amount<=0)return error(c,400,'VALIDATION_ERROR','Valor inválido');await db.insert(restaurantPayments).values({id:uuidv7(),businessId:b,tabId:id,method:String(body.method||'dinheiro'),amount:amount.toFixed(2),status:'confirmed',externalReference:body.externalReference?String(body.externalReference):null});await db.update(restaurantTabs).set({status:'payment_processing',updatedAt:new Date()}).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b)));return c.json({data:{ok:true}})});
restaurantRoutes.post('/tabs/:id/close',async c=>{
 const b=c.get('auth').businessId,id=c.req.param('id'),db=c.get('db'),user=c.get('auth').userId;
 const tab=await db.select().from(restaurantTabs).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b))).limit(1);if(!tab[0])return error(c,404,'NOT_FOUND','Comanda não encontrada');if(tab[0].saleId)return c.json({data:{saleId:tab[0].saleId,alreadyClosed:true}});
 const pays=await db.select().from(restaurantPayments).where(and(eq(restaurantPayments.businessId,b),eq(restaurantPayments.tabId,id),eq(restaurantPayments.status,'confirmed')));if(pays.reduce((s,p)=>s+n(p.amount),0)+0.001<n(tab[0].total))return error(c,409,'PAYMENT_INCOMPLETE','Pagamento ainda não cobre o total da comanda');
 const saleId=uuidv7();await db.insert(sales).values({id:saleId,businessId:b,customerId:tab[0].customerId,sellerUserId:user,status:'completed',subtotal:String(tab[0].subtotal),discount:String(tab[0].discount),total:String(tab[0].total),notes:`Comanda ${tab[0].code}`,completedAt:new Date()});
 const orders=await db.select({id:restaurantOrders.id}).from(restaurantOrders).where(and(eq(restaurantOrders.businessId,b),eq(restaurantOrders.tabId,id)));const items=orders.length?await db.select().from(restaurantOrderItems).where(and(eq(restaurantOrderItems.businessId,b),inArray(restaurantOrderItems.orderId,orders.map(o=>o.id)))):[];
 for(const i of items){await db.insert(saleItems).values({id:uuidv7(),businessId:b,saleId,itemType:i.productId?'product':'other',productId:i.productId,description:i.description,quantity:String(i.quantity),unitPrice:String(i.unitPrice),discount:'0',total:String(i.total)});if(i.productId)await db.insert(inventoryMovements).values({id:uuidv7(),businessId:b,productId:i.productId,movementType:'sale',quantity:String(-n(i.quantity)),referenceType:'sale',referenceId:saleId,createdBy:user});}
 for(const p of pays)await db.insert(salePayments).values({id:uuidv7(),businessId:b,saleId,method:p.method,amount:String(p.amount),status:'paid',externalReference:p.externalReference,paidAt:p.paidAt});
 const openCash=await db.select().from(cashSessions).where(and(eq(cashSessions.businessId,b),eq(cashSessions.status,'open'))).limit(1);
 if(openCash[0])for(const p of pays)await db.insert(cashMovements).values({id:uuidv7(),businessId:b,cashSessionId:openCash[0].id,movementType:'sale',amount:String(p.amount),referenceType:'sale',referenceId:saleId,description:`Comanda ${tab[0].code} - ${p.method}`,createdBy:user});
 await db.update(restaurantTabs).set({status:'closed',saleId,closedAt:new Date(),updatedAt:new Date()}).where(and(eq(restaurantTabs.id,id),eq(restaurantTabs.businessId,b)));if(tab[0].tableId)await db.update(restaurantTables).set({status:'free',currentTabId:null,occupiedAt:null,updatedAt:new Date()}).where(and(eq(restaurantTables.id,tab[0].tableId),eq(restaurantTables.businessId,b)));return c.json({data:{saleId}})
});