...
salesRoutes.post('/sales/:id/cancel',requirePermission('sales.cancel'),async c=>{const saleId=z.uuid().safeParse(c.req.param('id'));const p=z.object({reason:z.string().trim().min(3).max(1000)}).safeParse(await c.req.json().catch(()=>null));if(!saleId.success||!p.success)return error(c,422,'VALIDATION_ERROR','Cancelamento inválido');const result=await c.get('db').execute(sql`update sales set status='cancelled',cancelled_at=now(),cancelled_by=${c.get('auth').userId}::uuid,cancellation_reason=${p.data.reason} where id=${saleId.data}::uuid and business_id=${c.get('auth').businessId}::uuid and status='completed' returning id,total`);const sale:any=rows(result)[0];if(!sale)return error(c,409,'SALE_NOT_CANCELLABLE','Venda não encontrada ou já cancelada');await c.get('db').execute(sql`insert into audit_logs(id,business_id,actor_user_id,action,entity_type,entity_id,request_id,after_json) values(${uuidv7()},${c.get('auth').businessId}::uuid,${c.get('auth').userId}::uuid,'sale.cancelled','sale',${saleId.data}::uuid,${c.get('requestId')},${JSON.stringify({reason:p.data.reason,total:sale.total})}::jsonb)`);return c.json({data:{id:saleId.data,status:'cancelled'}});});
