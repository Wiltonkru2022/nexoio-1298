...
export function RegisterPage(){
 ...
 const submit=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();setLoading(true);setMessage('');const d=new FormData(e.currentTarget);const email=String(d.get('email')??'').trim();const callbackURL=`${location.origin}/login?verified=1`;try{
   const result=await api.post<{token?:string|null}>('/api/auth/sign-up/email',{name:String(d.get('name')??'').trim(),email,password,callbackURL});
   // Sem verificação obrigatória, Better Auth devolve uma sessão/token e seguimos
   // direto para o onboarding. Com verificação ativa, mostramos a etapa do e-mail.
   location.assign(result.token?'/':`/verificar-email?email=${encodeURIComponent(email)}`);
