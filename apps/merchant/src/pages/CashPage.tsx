...
export function CashPage(){
 const [data,setData]=useState<CashData>({session:null,movements:[],summary:{opening:0,entries:0,exits:0,balance:0}}),[loading,setLoading]=useState(true),[openModal,setOpenModal]=useState(false),[moveModal,setMoveModal]=useState(false),[reportOpen,setReportOpen]=useState(false),[error,setError]=useState('');
 ...
 const closeReport=(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setReportOpen(false)};
 return <><PageHeader title="Caixa" description="Abra, acompanhe e feche o caixa com movimentações auditáveis." action={<div style={{display:'flex',gap:10}}>{isOpen?<><button className="cash-report-reference" onClick={()=>setReportOpen(true)}>Ver Relatório</button><button className="cash-close-reference" onClick={()=>void close()}>Fechar Caixa</button></>:<Button onClick={()=>setOpenModal(true)}>Abrir caixa</Button>}</div>}/>
 ...
 <ContentCard title="Sessão atual" description="Movimentos do caixa em operação.">{error?<div className="restaurant-error">{error}</div>:null}{loading?<p>Carregando...</p>:rows.length?<div className="table-shell"><table><thead><tr><th>Data</th><th>Descrição</th><th>Forma / Tipo</th><th>Valor</th></tr></thead><tbody>{rows.map(x=>{const out=['withdrawal','expense','refund','out'].includes(x.movementType);return <tr key={x.id}><td>{new Date(x.createdAt).toLocaleString('pt-BR')}</td><td>{x.description||labels[x.movementType]||x.movementType}</td><td>{labels[x.movementType]||x.movementType}</td><td><strong style={{color:out?'#b42318':'#23884a'}}>{out?'-':'+'} {brl(Math.abs(Number(x.amount)))}</strong></td></tr>})}</tbody></table></div>:<div className="merchant-empty"><Pill tone={isOpen?'success':'warning'}>{isOpen?'Caixa aberto':'Caixa fechado'}</Pill><h3>{isOpen?'Nenhuma movimentação nesta sessão':'Abra o caixa para começar'}</h3></div>}{isOpen?<div className="card-footer"><Button ghost onClick={()=>setMoveModal(true)}>Nova movimentação</Button></div>:null}</ContentCard>
 ...
 <Modal open={reportOpen} onClose={()=>setReportOpen(false)} onSubmit={closeReport} title="Relatório do caixa" submitLabel="Fechar relatório"><div className="cash-report-grid"><div><span>Saldo inicial</span><strong>{brl(data.summary.opening)}</strong></div><div><span>Entradas</span><strong>{brl(data.summary.entries)}</strong></div><div><span>Saídas</span><strong>{brl(data.summary.exits)}</strong></div><div><span>Saldo esperado</span><strong>{brl(data.summary.balance)}</strong></div></div><p>Movimentações registradas: <b>{rows.length}</b></p></Modal></>;
}
