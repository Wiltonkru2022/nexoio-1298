...
export function RestaurantMenuPage(){
  ...
  const submit=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();const f=new FormData(e.currentTarget);setSaving(true);setError('');try{
    const stockControlEnabled=f.get('stockControlEnabled')==='on';
  ...
  return <>
    <PageHeader title="Cardápio" description="Itens, preços, fotos, disponibilidade e controle de estoque opcional." action={<Button onClick={openCreate}>Novo item</Button>}/>
    <section className="menu-panel"><div className="menu-panel-head"><h2>Itens do Cardápio</h2><div className="restaurant-toolbar"><button className={filter==='all'?'active':''} onClick={()=>setFilter('all')}>Todos</button><button className={filter==='active'?'active':''} onClick={()=>setFilter('active')}>Ativos</button><button className={filter==='inactive'?'active':''} onClick={()=>setFilter('inactive')}>Inativos</button><button onClick={()=>void load()}>Atualizar</button></div></div>
      ...
      {loading?<div className="restaurant-loading">Carregando cardápio…</div>:visible.length?<div className="menu-card-grid">{visible.map((p,i)=><article className={`menu-item-card ${p.active?'':'menu-item-inactive'}`} key={p.id}>
        ...
        <div><small>{p.sku||`C${String(i+1).padStart(2,'0')}`}</small><h3>{p.name}</h3><p>{p.description||'Item do cardápio'}</p><small>{p.stock_control_enabled?'Estoque controlado':'Sem controle de estoque'}</small></div>
    ...
    <Modal open={open} onClose={()=>!saving&&(setOpen(false),setEditing(null))} onSubmit={submit} title={editing?'Editar item':'Novo item'} description="A foto será armazenada no R2. Ative estoque apenas para itens que realmente precisam de saldo físico." submitLabel={saving?'Salvando…':'Salvar'}>
      ...
      <small className="menu-upload-help">Para pratos preparados, lanches e serviços normalmente deixe desativado. Ative para bebidas, produtos embalados ou itens com estoque real.</small>
