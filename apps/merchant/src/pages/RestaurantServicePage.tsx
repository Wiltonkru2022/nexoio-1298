...
const elapsed=(date?:string)=>{if(!date)return'agora';const m=Math.max(0,Math.floor((Date.now()-new Date(d).getTime())/60000));return m<60?`${m} min`:`${Math.floor(m/60)}h ${m%60}min`;};
...
export function RestaurantServicePage(){
  ...
  return <>
    ...
    {!standalone?<section className="service-history"><div className="service-history-head"><div><h2>Consumo da conta</h2><p>Todos os pedidos enviados durante este atendimento.</p></div><strong>{brl(accountTotal)}</strong></div>{accountOrders.length?<div className="service-order-list">{accountOrders.map(order=><article key={order.id}><div className="service-order-title"><div><strong>Pedido #{String(order.id).slice(-4).toUpperCase()}</strong><span>{elapsed(d(order.created_at))} · {order.fulfillment_status||order.status}</span></div><b>{brl(order.total)}</b></div><div className="service-order-items">{(order.items??[]).map(item=><span key={item.id}>{Number(item.quantity)}× {item.description}{item.notes?` · ${item.notes}`:''}</span>)}</div></article>)}</div>:<div className="service-history-empty">Ainda não há pedidos enviados nesta conta.</div>}</section>:null}
  </>;
}
