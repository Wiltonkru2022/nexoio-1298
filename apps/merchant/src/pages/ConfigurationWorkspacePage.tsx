...
export function ConfigurationWorkspacePage(){
 ...
 return <><PageHeader title="Configurações" description="Empresa, horários, operação, assinatura e segurança em um único lugar."/><div className="row-actions" role="tablist" aria-label="Seções de configurações"><Button onClick={()=>select('business')} variant={tab==='business'?'primary':'secondary'}>Empresa</Button><Button onClick={()=>select('hours')} variant={tab==='hours'?'primary':'secondary'}>Horários</Button><Button onClick={()=>select('system')} variant={tab==='system'?'primary':'secondary'}>Sistema e segurança</Button></div>{tab==='business'?<BusinessSettingsPage embedded/>:tab==='hours'?<BusinessHoursSection/>:<SettingsPage embedded/>}</>;
}
